...
static int synaptics_dsx_pinctrl_init(struct device *dev)
{
	int retval;
	...
	pinctrl_state_active
		= pinctrl_lookup_state(pinctrl, "pmx_ts_active");
	...
	pinctrl_state_suspend
		= pinctrl_lookup_state(pinctrl, "pmx_ts_suspend");
		...
		const struct i2c_device_id *dev_id)
{
    ...
    retval = synaptics_dsx_pinctrl_init(&client->dev);
    if (!retval && pinctrl) {
        retval = pinctrl_select_state(pinctrl,
                pinctrl_state_active);
        }
