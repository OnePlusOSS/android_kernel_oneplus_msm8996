...
int ctech_move_freepages_block(struct zone *zone, struct page *page, int migratetype, int old_mt);
...
static inline void fp_usage_add(int order) {if (alloc_status) atomic64_add(1, &fp_order_usage[order]);}
static inline void fp_fail_add(int order) {if (alloc_status) atomic64_add(1, &fp_order_fail[order]);}
